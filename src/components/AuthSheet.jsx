...
//
// ⚠ ИМПОРТ ПОД УСЛОВИЕМ, И ЭТО НЕ ПРИДИРКА К СТИЛЮ.
//
// web3Enabled вычисляется из import.meta.env, а Vite подставляет значение
// переменной окружения в код НА СБОРКЕ. Значит при незаданной
// VITE_WALLETCONNECT_PROJECT_ID условие становится константой false, и Rollup
// выбрасывает ветку вместе с динамическим импортом — чанк на 2,7 МБ просто не
// собирается.
//
// Раньше импорт стоял безусловно, и чанк собирался ВСЕГДА: его никто не
// загружал (кнопка не рисовалась), но он лежал в сборке, попадал в артефакты
// развёртывания и на каждой сборке вызывал предупреждение о размере.
//
// Когда переменная задана, всё работает ровно как прежде: ленивый импорт,
// граница ошибок, заглушка на время загрузки.
const Web3Button = __WEB3_ENABLED__
  ? lazyWithReload(() => import('./Web3Button.jsx'))
  : null
