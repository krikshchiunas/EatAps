...
export function normalizeError(err) {
  ...
  // Неопознанная ошибка — единственный случай, когда человеку показывают
  // технический код.
  //
  // Обычно код в интерфейсе — признак лени: «ошибка 500» ничего не объясняет и
  // ничего не предлагает. Но здесь ситуация обратная. Все ОБЪЯСНИМЫЕ ошибки
  // разобраны правилами выше и получают человеческий текст; сюда попадает
  // ровно то, чего мы не предусмотрели. Без кода такая ошибка неотличима от
  // любой другой: и на скриншоте с телефона, и в рассказе «просто не
  // работает». Пять символов SQLSTATE превращают это в «код 42P10» — по нему
  // причина находится за минуту.
  //
  // Безопасно: SQLSTATE и коды PostgREST — это класс ошибки, а не её
  // содержимое. Ни имён таблиц, ни колонок, ни данных в них нет; сам текст
  // ошибки базы наружу по-прежнему не уходит и живёт только в консоли.
  const code = err?.code ? String(err.code) : ''
  const suffix = code && code !== 'unknown' ? ` (код ${code})` : ''
  return {
    category: ERR.UNKNOWN,
    message: `Что-то пошло не так. Попробуйте ещё раз${suffix}`,
    retryable: false,
    code: code || 'unknown',
  }
}
