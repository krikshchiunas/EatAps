...
export const NUTRIENTS = [
  ...
  {
    key: 'qualityProtein', label: 'Усвояемый белок', short: 'Усв. белок', unit: 'г', color: 'var(--good)', estimate: true,
    disclaimer: 'Белок, взвешенный по DIAAS — международному показателю качества белка (FAO): каждый продукт засчитывается в той доле, в какой его белок пригоден для строительства тканей. Значения справочные, по типу продукта, а не лабораторные данные конкретной пачки.',
  },
...
// Настоящий сахар записи — или null, если его не знают.
//
// Ноль здесь двусмыслен, и различить два смысла можно только по пометке:
//   • sugarSrc: 'measured' — человек (или этикетка) сказал «ноль». У масла и
//     мяса сахара действительно нет, и подставлять вместо этого оценку из
//     углеводов было бы прямым враньём.
//   • без пометки — поле просто не заполнено. Так писали старые версии
//     приложения, и все накопленные записи с sugar: 0 означают именно это.
// Поэтому пометка обязательна: без неё ноль по-прежнему считается пробелом, и
// уже сохранённые дневники не начинают внезапно показывать другие цифры.
export const SUGAR_MEASURED = 'measured'

export function realSugar(m) {
  const n = Number(m?.sugar)
  if (!Number.isFinite(n) || n < 0) return null
  if (n > 0) return n
  return m?.sugarSrc === SUGAR_MEASURED ? 0 : null
}
