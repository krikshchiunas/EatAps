...
// isFavorite(favorites, food) здесь больше нет — и возвращать его не стоит.
//
// Функция была не просто мёртвой, а вредной: линейный проход по списку с
// вычислением ключа на каждом шаге. Единственное место, где проверка нужна, —
// отрисовка звёздочек у результатов поиска, то есть внутри цикла по сотням
// строк; там это O(n·m). Экран добавления строит Set ключей один раз и
// спрашивает его за O(1). Удобный, но медленный помощник рядом с быстрым
// решением рано или поздно попадёт в цикл.
