...
test('принятые добавки в карточку не попадают', () => {
  // Что человек пьёт — это здоровье, а не рацион: мелатонин, витамины для
  // беременных или 5-HTP рассказывают о нём больше, чем вес, который мы уже
  // договорились не публиковать. Карточка читает только day.meals, и этот тест
  // существует, чтобы добавки туда не «дописали заодно» при следующей правке.
  const d = buildShareData({
    meals: [meal({ name: 'Овсянка', kcal: 300 })],
    supps: [
      { id: 'p1', name: 'Мелатонин 3 мг', unit: 'таблетка', dose: 1, provides: { melatonin: 3 } },
      { id: 'p2', name: 'Витамины для беременных', unit: 'таблетка', dose: 1, provides: { b9: 600 } },
    ],
  }, '2026-08-23', { name: 'Денис' })

  const dump = JSON.stringify(d)
  assert.ok(!dump.includes('Мелатонин'), 'приём добавок публиковать нельзя')
  assert.ok(!dump.includes('беременных'), 'приём добавок публиковать нельзя')
  assert.ok(!dump.includes('melatonin'), 'состав добавок публиковать нельзя')
  assert.equal(d.mealCount, 1, 'добавки не должны считаться едой')
})
