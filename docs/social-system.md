...
Старая переписка после блокировки **не удаляется, но и не читается**: политика
`messages` прячет её, и то же условие продублировано внутри
`list_conversation_messages`, `conversation_media` и `search_messages`. Эти три
функции `SECURITY DEFINER`, то есть RLS их не касается, — без явной проверки
заблокированный человек, у которого сохранился id диалога, читал бы всю
историю через RPC, хотя прямой `select` ему уже ничего не отдаёт. Два разных
ответа на один вопрос — это и есть дыра, а не мелочь.
