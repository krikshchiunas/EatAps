...
// addAll отвергает весь запрос целиком, если хотя бы один адрес отдал не 200.
// То есть один отсутствующий значок означал бы, что service worker не
// установился вовсе и офлайн-режима нет совсем. Кладём поштучно: что удалось —
// закэшировано, чего нет — доберётся из сети по факту обращения.
self.addEventListener('install', (e) => {
  e.waitUntil(
    caches.open(CACHE).then((c) => Promise.all(ASSETS.map((url) => c.add(url).catch(() => {}))))
  )
})
