...
// ⚠ ЗАПАСНОГО ЗНАЧЕНИЯ ЗДЕСЬ БОЛЬШЕ НЕТ.
//
// Раньше стояло `process.env.TG_ADMIN_CHAT_IDS || '571138125,938539456'` —
// личные идентификаторы владельца прямо в исходниках, и они же в истории
// репозитория. Права это не давало (бот пишет В эти чаты, а не слушает их),
// но раскрывало личный идентификатор и, что хуже, делало конфигурацию
// двусмысленной: при незаданной переменной сообщения молча уходили бы в
// зашитый чат.
//
// Пустой список означает «администраторов нет»: бот никому не отвечает и
// никого не банит. Это правильный отказ — лучше неработающая модерация, чем
// модерация, отвечающая непонятно кому.
export const ADMIN_CHAT_IDS = String(process.env.TG_ADMIN_CHAT_IDS || '')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean)
  .map(Number)
...
export function isAdmin(chatId) {
  // Пустой список не делает администратором никого — включая того, кто
  // прислал запрос. Проверка на пустоту явная, чтобы это нельзя было
  // случайно «оптимизировать» до includes() по пустому массиву.
