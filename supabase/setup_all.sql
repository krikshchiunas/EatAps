...
--
-- Что это: schema.sql и все семь миграций, склеенные в правильном порядке.
...
--
-- После прогона выполните supabase/verify.sql (проверки синхронизации) и
-- supabase/verify_social.sql (проверки социального графа). Только чтение.
-- Все строки должны быть ✔.
--
-- ВНИМАНИЕ, миграция социального графа меняет модель приватности: имя,
-- аватар и username становятся видны любому авторизованному пользователю, а
-- существующие «Мысли» — подписчикам, а не только друзьям. Подробности и
-- обоснование — в шапке 2026-08-25_social_graph.sql. Перед прогоном на
-- продакшене обновите docs/compliance и уведомьте пользователей.
--
