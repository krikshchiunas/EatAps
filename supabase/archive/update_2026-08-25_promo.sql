-- ═══════════════════════════════════════════════════════════════════════════
-- ⛔ АРХИВ. ЭТОТ ФАЙЛ ЗАПУСКАТЬ НЕЛЬЗЯ.
--
-- Это снимок уже применённых миграций (лето 2026), а не отдельная работа.
-- Написан через create or replace / drop policy, поэтому его прогон СЕГОДНЯ
-- молча заменит живые объекты их старыми редакциями и откатит более поздние
-- исправления. Что именно откатит каждый файл — в supabase/archive/README.md.
--
-- Установка с нуля      → supabase/setup_all.sql
-- Обновление базы       → supabase/migrations/ по порядку
-- Что здесь происходило → supabase/docs/04-history.md
--
-- Блок ниже — предохранитель: он останавливает выполнение на первой же строке,
-- если файл всё-таки вставили в SQL Editor.
-- ═══════════════════════════════════════════════════════════════════════════

do $archived$
begin
  raise exception
    'Архивный файл. Запускать нельзя: он откатит более поздние исправления. Смотрите supabase/archive/README.md'
    using errcode = '55000';
end
$archived$;
