-- ═══════════════════════════════════════════════════════════════════════════
-- EatAps — ОБНОВЛЕНИЕ ОТ 2026-08-24: AI-ассистент. Вставить целиком в
...
--
-- Что появится:
--   • ai_usage      — расход токенов ассистента: строка на пользователя и
--                     календарный месяц UTC, суммы в целых микродолларах;
--   • ai_usage_add  — атомарное списание и возврат резерва.
--
-- Существующие таблицы, политики и функции НЕ затрагиваются: здесь нет ни
-- одного оператора, который меняет что-то за пределами этих двух объектов.
-- Прав у клиента не прибавляется — он получает только чтение собственной
-- строки расхода.
--
-- Безопасно для базы с данными и для повторного прогона.
--
-- ⚠️ ЕСЛИ ВЫ УЖЕ ПРОГОНЯЛИ РАННЮЮ ВЕРСИЮ ЭТОГО ФАЙЛА — прогоните ещё раз.
-- У ai_usage_add сменилась сигнатура (добавился p_count), и старая
-- трёхаргументная версия сносится явным drop: без этого рядом остались бы две
-- перегрузки, а вызов с тремя аргументами стал бы неоднозначным для Postgres.
-- Накопленный расход при этом сохраняется — таблица не пересоздаётся.
--
-- После прогона выполните supabase/verify.sql — там появились проверки 70–76.
--
-- Собран из этого файла, править нужно ЕГО:
